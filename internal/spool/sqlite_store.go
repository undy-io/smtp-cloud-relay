...
const (
	...
	spoolProviderName    = "spool"
	sqliteBusyTimeoutMS  = 5000
	sqliteDriverName     = "sqlite"
	sqliteJournalModeWAL = "WAL"
)

// SQLiteStore persists spool state in SQLite and message payloads on disk.
...
func NewSQLiteStore(root string) (*SQLiteStore, error) {
	...
	payloads, err := NewPayloadStore(cleanRoot)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("open spool sqlite database: %w", err)
	}
...
func (s *SQLiteStore) Enqueue(ctx context.Context, msg email.Message) (Record, error) {
	...
	id, err := s.newID()
	if err != nil {
		return Record{}, fmt.Errorf("generate spool record id: %w", err)
	}
...
func (s *SQLiteStore) ClaimReady(ctx context.Context, now time.Time) (Record, bool, error) {
	...
	now = now.UTC()
	for {
		meta, ok, err := s.claimReadyMetadata(ctx, now)
		if err != nil || !ok {
		...
		if err == nil {
			return meta.toRecord(msg), true, nil
		}

		if _, corrupt := AsPayloadCorruptionError(err); corrupt {
			if _, deadErr := s.deadLetterBrokenPayload(ctx, meta, err); deadErr != nil {
				return Record{}, false, deadErr
			}
			continue
		}
		if requeueErr := s.requeueClaimedRecord(ctx, meta); requeueErr != nil {
...
func (s *SQLiteStore) Recover(ctx context.Context, now time.Time) (RecoveryResult, error) {
	...
	now = now.UTC()
	working, submitted, validIDs, err := s.recoverMetadata(ctx, now)
	if err != nil {
		return RecoveryResult{}, err
	}

	result := RecoveryResult{
		Requeued:     make([]Record, 0, len(working)),
		Submitted:    make([]Record, 0, len(submitted)),
	...
	for _, meta := range working {
		...
		if loadErr != nil {
			if _, corrupt := AsPayloadCorruptionError(loadErr); corrupt {
				dead, err := s.deadLetterBrokenPayload(ctx, meta, loadErr)
				if err != nil {
					return RecoveryResult{}, err
				}
			...
			return RecoveryResult{}, fmt.Errorf("load requeued payload %q during recovery: %w", meta.ID, loadErr)
		}
		result.Requeued = append(result.Requeued, meta.toRecord(msg))
	}

	for _, meta := range submitted {
		msg, loadErr := s.payloads.Load(meta.ID)
		if loadErr != nil {
			if _, corrupt := AsPayloadCorruptionError(loadErr); corrupt {
				dead, err := s.deadLetterBrokenPayload(ctx, meta, loadErr)
			...
			return RecoveryResult{}, fmt.Errorf("load submitted payload %q during recovery: %w", meta.ID, loadErr)
		}
		result.Submitted = append(result.Submitted, meta.toRecord(msg))
	}

	result.OrphanedPayloads, err = s.payloads.QuarantineOrphans(validIDs)
	if err != nil {
		return RecoveryResult{}, err
	}
...
func (s *SQLiteStore) insertRecord(ctx context.Context, rec Record) error {
	...
	if _, err := conn.ExecContext(ctx, `
		...
		return fmt.Errorf("insert spool record %q: %w", rec.ID, err)
	}
...
func (s *SQLiteStore) claimReadyMetadata(ctx context.Context, now time.Time) (recordMetadata, bool, error) {
	conn, err := s.beginImmediate(ctx)
	if err != nil {
	...
	if err != nil {
		return recordMetadata{}, false, fmt.Errorf("claim spool record %q: %w", meta.ID, err)
	}
	rowsAffected, err := result.RowsAffected()
	if err != nil {
		return recordMetadata{}, false, fmt.Errorf("read claim result for %q: %w", meta.ID, err)
	}
	if rowsAffected != 1 {
...
func (s *SQLiteStore) updateRecord(ctx context.Context, currentState State, updated Record) error {
	...
	if err != nil {
		return fmt.Errorf("update spool record %q: %w", updated.ID, err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("read spool update result for %q: %w", updated.ID, err)
	}
	if rows != 1 {
...
func (s *SQLiteStore) requeueClaimedRecord(ctx context.Context, meta recordMetadata) error {
	updated := meta.toRecord(email.Message{})
	updated.State = StateQueued
	updated.UpdatedAt = s.now().UTC()
	if err := s.updateRecord(ctx, meta.State, updated); err != nil {
		return fmt.Errorf("requeue claimed spool record %q after payload load failure: %w", meta.ID, err)
	}
	return nil
}

func (s *SQLiteStore) recoverMetadata(ctx context.Context, now time.Time) ([]recordMetadata, []recordMetadata, map[string]struct{}, error) {
	conn, err := s.beginImmediate(ctx)
	if err != nil {
	...
	for _, meta := range working {
		...
		if _, err := conn.ExecContext(ctx, `
			...
			return nil, nil, nil, fmt.Errorf("requeue stale working record %q: %w", requeuedMeta.ID, err)
		}
	...
	committed = true
	return requeued, submitted, validIDs, nil
}

func (s *SQLiteStore) deadLetterBrokenPayload(ctx context.Context, meta recordMetadata, cause error) (Record, error) {
	now := s.now().UTC()
	updated := meta
	updated.State = StateDeadLetter
	...
	rec := updated.toRecord(email.Message{})
...
func (s *SQLiteStore) bootstrapSchema(ctx context.Context) error {
	...
	if _, err := conn.ExecContext(ctx, recordsTableSchema); err != nil {
		return fmt.Errorf("create current spool schema: %w", err)
	}

	for _, stmt := range []string{recordsReadyIndexSchema, recordsOperationIndexSchema} {
		if _, err := conn.ExecContext(ctx, stmt); err != nil {
			return fmt.Errorf("create spool index: %w", err)
		}
	}
...
func tableExists(ctx context.Context, db *sql.DB, name string) (bool, error) {
	...
	if err := db.QueryRowContext(
		...
		return false, fmt.Errorf("query sqlite table %q: %w", name, err)
	}
	return count == 1, nil
}
...
func loadTableColumns(ctx context.Context, db *sql.DB, table string) ([]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("query sqlite table info for %q: %w", table, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cid, &name, &columnType, &notNull, &defaultVal, &pk); err != nil {
			return nil, fmt.Errorf("scan sqlite table info for %q: %w", table, err)
		}
		columns = append(columns, name)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate sqlite table info for %q: %w", table, err)
	}
	return columns, nil
}
...
func tableSQL(ctx context.Context, db *sql.DB, table string) (string, error) {
	...
	if err := db.QueryRowContext(
		...
		return "", fmt.Errorf("query sqlite sql for table %q: %w", table, err)
	}
...
func indexSQL(ctx context.Context, db *sql.DB, index string) (string, error) {
	...
	if err := db.QueryRowContext(
		...
		return "", fmt.Errorf("query sqlite sql for index %q: %w", index, err)
	}
	if !sqlText.Valid || strings.TrimSpace(sqlText.String) == "" {
		return "", fmt.Errorf("sqlite index %q has no schema SQL", index)
	}
...
func (s *SQLiteStore) finishImmediate(ctx context.Context, conn *sql.Conn, committed *bool) {
	if conn == nil {
		return
	}
...
func queryOneMetadata(ctx context.Context, conn *sql.Conn, query string, args ...any) (recordMetadata, bool, error) {
	...
	if err != nil {
		return recordMetadata{}, false, fmt.Errorf("query spool metadata: %w", err)
	}
	defer rows.Close()
	if !rows.Next() {
		if err := rows.Err(); err != nil {
			return recordMetadata{}, false, fmt.Errorf("iterate spool metadata: %w", err)
		}
...
func queryAllMetadata(ctx context.Context, conn *sql.Conn, query string, args ...any) ([]recordMetadata, error) {
	rows, err := conn.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("query spool metadata: %w", err)
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate spool metadata: %w", err)
	}
	return out, nil
}
...
func queryAllIDs(ctx context.Context, conn *sql.Conn) (map[string]struct{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("query spool ids: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id); err != nil {
			return nil, fmt.Errorf("scan spool id: %w", err)
		}
		if !id.Valid {
			return nil, fmt.Errorf("spool record row missing id")
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate spool ids: %w", err)
	}
	return out, nil
}
...
func scanMetadata(scanner interface {
	...
	if err := scanner.Scan(
		...
		return recordMetadata{}, fmt.Errorf("scan spool metadata: %w", err)
	}

	if !id.Valid {
		return recordMetadata{}, fmt.Errorf("spool record row missing id")
	}
	meta.ID = id.String
	...
	if !isValidState(meta.State) {
		return recordMetadata{}, fmt.Errorf("spool record %q has invalid state %q", meta.ID, meta.State)
	}
	if meta.Attempt < 0 {
		return recordMetadata{}, fmt.Errorf("spool record %q has negative attempt %d", meta.ID, meta.Attempt)
	}
	if nextAttemptAtMS <= 0 {
		return recordMetadata{}, fmt.Errorf("spool record %q has invalid next_attempt_at_ms %d", meta.ID, nextAttemptAtMS)
	}
	if createdAtMS <= 0 {
		return recordMetadata{}, fmt.Errorf("spool record %q has invalid created_at_ms %d", meta.ID, createdAtMS)
	}
	if updatedAtMS <= 0 {
		return recordMetadata{}, fmt.Errorf("spool record %q has invalid updated_at_ms %d", meta.ID, updatedAtMS)
	}
	...
	if meta.UpdatedAt.Before(meta.CreatedAt) {
		return recordMetadata{}, fmt.Errorf("spool record %q has updated_at before created_at", meta.ID)
	}
	if lastErrMessage.Valid || lastErrProvider.Valid || lastErrTemporary.Valid || lastErrTimestampMS.Valid {
		if lastErrTemporary.Valid && lastErrTemporary.Int64 != 0 && lastErrTemporary.Int64 != 1 {
			return recordMetadata{}, fmt.Errorf("spool record %q has invalid last_error_temporary %d", meta.ID, lastErrTemporary.Int64)
		}
		if !lastErrTimestampMS.Valid || lastErrTimestampMS.Int64 <= 0 {
			return recordMetadata{}, fmt.Errorf("spool record %q has incomplete last error timestamp state", meta.ID)
		}
...
func (m recordMetadata) toRecord(msg email.Message) Record {
	return Record{
		ID:                m.ID,
		Message:           msg,
		State:             m.State,
...
func lastErrorProvider(lastErr *LastError) any {
	...
	return nullIfEmpty(lastErr.Provider)
}
...
func lastErrorTimestamp(lastErr *LastError) any {
	...
	return timeToMillis(lastErr.Timestamp)
}
