...
(
  ...
  git checkout -b "${main_branch}" >/dev/null

  printf 'first\n' > tracked.txt
  git add tracked.txt
  git commit -m "first" >/dev/null
  git push origin HEAD:"${main_branch}" >/dev/null
  ...
  stale_log="${tmp_dir}/stale-default-branch.log"
  if GITHUB_REF="refs/heads/${main_branch}" \
    GITHUB_SHA="${stale_sha}" \
      bash "${verify_main_tip_copy}" >"${stale_log}" 2>&1; then
    echo "expected stale default-branch guard to fail" >&2
    exit 1
  fi

  assert_contains "${stale_log}" "refusing to publish refs/heads/${main_branch}"
